...
> **Amendment (2026-07-23): phases 2–3 superseded by threshold compaction, implemented.** The 2026-07-23 context measurements changed the picture in two ways. (a) Any per-turn or per-batch mutation of re-sent history invalidates the KV cache at the mutation point — repeatedly. Steady-state growth with a *warm* cache turned out to be the cheap case; invalidation is the expensive one. (b) The dominant ballast was not old tool *results* but old **Write/Edit arguments** (the agent carries every file it wrote), a target phase 2 didn't know. The implemented design (Bruce's, mirroring Claude Code's own `/compact`): the steady state never mutates the transcript; when the last observed prompt size crosses `compactThreshold` (default 0.9) of the profile-declared `contextWindow`, the run pays **one** deliberate reset — the model summarizes its own session from a ballast-stripped one-shot input (stripping only in that input, never in the live transcript; this is where phase 2's stubbing survives), the summary lands as an append-only `_compact` marker message — which also carries the last few exchanges **as inline text** (Bruce's design: packing them into the marker sidesteps every alternation constraint that carrying them as real messages would raise — orphaned tool results, consecutive user turns; capped so a huge final tool result can't smuggle the ballast back) — `buildMessages` sends system prefix + everything from the newest marker, and the ADR 0013 knowledge state is cleared so the gates force re-reads. Opt-in via `contextWindow` (the API doesn't expose it); a server that omits `usage` never triggers. `lib/compaction.mjs`, covered by `tests/compaction.test.mjs`. The phase-3 open question (summarisation model) resolved to: same profile, tool-less call — the first ADR 0016-shaped harness call. The related knowledge-state gap closed the same day: `visitedDirs`/`pendingDirMds` moved from process-local to thread meta, and `knownFiles` grew `fileStamps` ({mtimeMs, size, full} — stale-Write gate + unchanged-full-re-read dedup).
