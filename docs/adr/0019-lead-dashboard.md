...
- **Status:** Accepted — phase 1 implemented (2026-07-24, after the second field incident: a thread wedged 43 min on one LLM call at 18× its own average, killed by hand). Shipped: `lib/status.mjs` (the judgment layer — `deriveInFlight`, `deriveActivity`, `judgeThread`, `fmtDur`); `show` carries `verdict`/`evidence`/`action` per the glossary; the hook block renders the `hung` alert line (self-contained mirror in `scripts/hooks/_lib.mjs`: `readRunState`/`isHungRunState`); §7 partially — agnz publishes `from:"agnz"` errors for a dead runner (previously silent recovery) and a failed compaction (previously trace-only). **Phase 1b (2026-07-24, from the first structured lead field report):** the `activity` liveness triple `{phase: generating|tool|idle, since, last_action}` in `show`/`wait` + the block's `· generating Xs` tag — the tool-call-only `lastActivity` froze during long generations and "generiert" was indistinguishable from "hängt" (cost the field lead a night of hand-built PID watching); `turn-limit` verdict + ⚠ block marker (a flail-out must not read like a clean finish); `filesTouched` in `show <id>` (the reviewing lead's diff pointer); the `agent`→`role` rename in CLI outputs (glossary §6); show de-noised (agentDef → `role` only; `recent` filtered of the lead's own user-role directives — a 3.6 KB self-echo was observed); the `mailbox` verb (read-only log peek — agent-to-agent traffic had no interface). Not yet: full `collectStatus` struct with both renderers on it, the severity-gated one-line healthy block, workspace-level preflight/server-contact lines.
- **Date:** 2026-07-23
