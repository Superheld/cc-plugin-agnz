// Zero-dep aggregator over the unified per-thread log (ADR 0020; formerly
// the ADR 0011 trace.jsonl, which no longer exists).
//
...
const TRACE_SUFFIX = ".log.jsonl";

/**
 * Which files did this thread mutate? Folds successful Write/Edit tool calls
 * into per-path counts — the diff POINTER a reviewing lead needs ("das hat er
 * editiert"), where toolCalls.byName only says "er hat editiert". Line-level
 * detail stays with `git diff`; this tells you where to point it. Bash-side
 * mutations are invisible here by nature — the trace has no target for them.
 *
 * Returns e.g. ["lib/foo.mjs (1 write, 3 edits)"] in first-touched order.
 */
export function filesTouched(entries) {
  const byPath = new Map();
  for (const e of entries || []) {
    if (!e || e.type !== "tool_exec" || typeof e.target !== "string") continue;
    if (e.name !== "Write" && e.name !== "Edit") continue;
    if (e.outcome !== "ok") continue;
    const rec = byPath.get(e.target) || { writes: 0, edits: 0 };
    if (e.name === "Write") rec.writes++;
    else rec.edits++;
    byPath.set(e.target, rec);
  }
  return [...byPath].map(([path, { writes, edits }]) => {
    const parts = [];
...
export function aggregateTrace(entries) {
  const s = {
    ...
    turns: 0,
    llmCalls: 0,
    llmErrors: 0, // calls that never reached the model (network, timeout, 4xx/5xx)
    llmLatencyMs: 0,
  ...
  for (const e of entries) {
    ...
    switch (e.type) {
      case "run_start":
        s.agent = e.agentDef ?? s.agent;
        s.model = e.model ?? s.model;
        s.profile = e.profile ?? s.profile;
        s.maxTurns = e.maxTurns ?? s.maxTurns;
        break;
      case "api_request":
        // Turns are counted from what was ASKED, not from a separate marker:
        // one request per turn, and a turn the model never answered still
        // happened. The old turn_start event existed only to carry this.
        s.turns += 1;
        break;
      case "api_error":
        s.llmErrors += 1;
        break;
      case "api_response":
        // api_error is counted above and deliberately never reaches here: a
        // call the model did not answer is not a call, and folding a 2 ms
        // connect refusal or a full timeout into the mean would wreck it.
        s.llmCalls += 1;
      ...
      case "tool_exec":
        ...
        break;
      case "harness":
        // `harness` covers every intervention (repair, compaction, …); only
        // repairs belong in the repair rate.
        if (e.kind === "repair") {
          s.repairs.total += 1;
...
export async function aggregateWorkspace(cwd) {
  ...
  for (const t of threads) {
    ...
    totals.llmErrors += t.llmErrors || 0;
...
export function formatThread(threadId, s) {
  ...
  lines.push(
    `llm:      ${s.llmCalls} calls, ${fmtMs(s.llmLatencyMs)} total (${fmtMs(s.avgLlmLatencyMs)} avg)` +
      (s.llmErrors ? `, ${s.llmErrors} failed` : ""),
  );
  lines.push(
