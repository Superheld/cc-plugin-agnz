...
export function filesTouched(entries) {
  ...
  for (const e of entries || []) {
    if (!e || e.type !== "tool_call" || typeof e.target !== "string") continue;
  ...
  for (const e of entries) {
    ...
    switch (e.type) {
      ...
      case "turn_start":
        ...
        break;
      case "llm_call":
        // Failed calls are on the same timeline now (a network error, a 4xx),
        // but they are not calls the model answered: folding their latency in
        // would drag the average toward either a 2 ms connect refusal or a
        // full timeout. Count them, keep them out of the latency mean.
        if (e.outcome === "error") {
          s.llmErrors += 1;
          break;
        }
