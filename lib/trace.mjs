...
//   turn_start    — before every LLM API call: just the turn number. Used to
//                   carry the current system prompt too, but since ADR 0012
//                   phase 1 froze the prefix (byte-identical every turn,
//                   reused from thread.systemPromptSnapshot) that would just
//                   duplicate thread_start's copy on every turn for nothing
//   llm_call      — around each chat(): latencyMs, finishReason, usage
//   tool_call     — around each tool dispatch: name, latencyMs, outcome
//                   (ok|error|denied|blocked), and `target` when the call has
//                   a path-ish argument (file path, command/pattern prefix) —
//                   powers the last-activity display on running threads
