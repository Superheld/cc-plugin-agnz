...
// The index is a single global file shared across every workspace AND across
// processes (each CLI invocation + each detached runner). register and forget
// are read-modify-write, so they must be serialised or a concurrent pair loses
// one entry — orphaning a live thread. The cross-process mkdir lock is keyed on
// the index path; we mkdir the parent first so the lock dir can be created.
...
export async function registerThread(threadId, cwd) {
  ...
  return withProcLock(indexFile() + ".lock", async () => {
