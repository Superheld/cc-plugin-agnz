...
import { openThreadLog } from "./run-log.mjs";
...
export function createThreadManager() {
  ...
  async function writerFor(id) {
    ...
    if (!cwd) return null;
    const key = `${id}:${runContext.runId}`;
    let w = logWriters.get(key);
    if (!w) {
      w = await openThreadLog({ cwd, threadId: id, runId: runContext.runId });
  ...
  async function appendMessage(id, message) {
    ...
    const w = await writerFor(id).catch(() => null);
    if (w) await w.append("message", message);
