...
import { openThreadLog, readThreadLog, projectMessages } from "./run-log.mjs";
...
export function createThreadManager() {
  ...
  // ---- unified log (ADR 0020) ----
  //
  // The run context is manager state rather than a parameter threaded through
  // every append site, because a runner process owns exactly one thread for
  // exactly one run (`claimThread` guarantees it) — so there is one correct
  // answer at any moment, and passing it through eight call sites would only
  // spread the same fact around. Writers are cached per (thread, run) so the
  // sequence numbers stay monotonic within a run.
  let runContext = { runId: null, turn: null };
  const logWriters = new Map();

  function setRunContext(ctx) {
    runContext = { ...runContext, ...ctx };
    const w = logWriters.get(`${ctx.threadId ?? ""}:${runContext.runId}`);
    if (w && typeof runContext.turn === "number") w.setTurn(runContext.turn);
  }

  // Appends outside a run (a caller seeding a transcript, a test) still belong
  // in the log — there is nowhere else for them to go now. They get this
  // process's fallback run id so every entry stays attributable to something.
  const fallbackRunId = randomUUID();

  async function writerFor(id) {
    const runId = runContext.runId || fallbackRunId;
    const cwd = cwdById.get(id);
    if (!cwd) return null;
    const key = `${id}:${runId}`;
    let w = logWriters.get(key);
    if (!w) {
      w = await openThreadLog({ cwd, threadId: id, runId });
      logWriters.set(key, w);
    }
    if (typeof runContext.turn === "number") w.setTurn(runContext.turn);
    return w;
  }

  /**
   * Append an observability event to the thread's log. Never rejects — the
   * run must not die because its record could not be written.
   * Returns the entry (for referencing it by seq) or null.
   */
  async function log(id, type, payload = {}) {
    const w = await writerFor(id).catch(() => null);
    if (!w) return null;
    return w.appendQuiet(type, payload);
  }

  /** The seq the thread's log will hand out next — for naming a range. */
  async function nextLogSeq(id) {
    const w = await writerFor(id).catch(() => null);
    return w ? w.nextSeq() : null;
  }

  /** Seq range covering the last `count` messages (ADR 0020 §3). */
  async function logRangeForLastMessages(id, count) {
    const w = await writerFor(id).catch(() => null);
    return w ? w.rangeForLastMessages(count) : null;
  }

  async function appendMessage(id, message) {
    const store = storeForThreadId(id);
    if (!store) throw new Error(`threads: no such thread: ${id}`);
    // The log is the record; <id>.jsonl is a write-only shadow during the
    // switch (ADR 0020 §7) so a `git revert` finds what the reverted code
    // expects — git reverts code, not data on disk. Reads still come from the
    // shadow until the projection has proven itself on real threads.
    const w = await writerFor(id);
    if (!w) throw new Error(`threads: cannot resolve a workspace for ${id}`);
    await w.append("message", message);
    await updateThread(id, {}); // bump updatedAt
  }

  // The conversation, projected out of the one log (ADR 0020). Threads created
  // before the log fall back to their old transcript so they keep running —
  // read compatibility without writing two files.
  async function readMessages(id) {
    const cwd = cwdById.get(id);
    if (!cwd) return [];
    const entries = await readThreadLog(cwd, id);
    if (entries.length > 0) return projectMessages(entries);
    const store = storeFor(cwd);
