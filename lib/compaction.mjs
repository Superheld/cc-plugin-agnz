...
export async function performCompaction({ thread, threadMgr, profile, chatFn, turn, signal }) {
  ...
  try {
    ...
    // CLAUDE.md.
    // `focus` joins the reset: the streak counted calls whose context is gone
    // after the summary, so carrying it over could pause a thread that is in
    // fact starting fresh.
    const reset = { knownFiles: [], fileStamps: {}, visitedDirs: [], pendingDirMds: [], focus: null };
