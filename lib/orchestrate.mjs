...
import { resolveProfileForModel } from "./config.mjs";
 ...
 * always re-derived from agentDef.model via the merged config (ADR 0017),
 * so profile/mapping changes take effect on the next run without touching
 * the thread.
 */
export async function resolveProfile(thread) {
  return resolveProfileForModel(thread.cwd, thread.agentDef?.model || "_default");
}
