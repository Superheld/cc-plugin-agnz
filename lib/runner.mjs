...
const { threadId, cwd = null, userMessage = null, resumeInput = null } = payload;
const threadMgr = createThreadManager();
const registry = createRegistry();

// cwd seeds id→cwd resolution — a fresh process has no in-memory map and
// there is no user-wide index any more (ADR 0017).
const thread = await threadMgr.getThread(threadId, cwd);
if (!thread) {
...
// Admission control (fixes the two-runner race). Atomically claim the thread
// before doing any work: if another live runner already owns it, we lost the
// race — write nothing, say so once, and exit cleanly without touching the
// transcript. This is the authoritative gate; the CLI-side guards are just UX.
const claimed = await threadMgr.claimThread(threadId, process.pid);
if (!claimed) {
  process.stderr.write(`runner: thread ${threadId} already claimed by a live runner — exiting\n`);
  process.exit(0);
}
