...
export function buildToolPolicy(agentDef, availableTools = []) {
  ...
  // Skill is auto-allowed when the agent has skills configured —
  // the agent needs to call it to load skill bodies on demand.
  if (Array.isArray(skills) && skills.length > 0 && out["Skill"] !== "deny") {
    out["Skill"] = "allow";
  }
