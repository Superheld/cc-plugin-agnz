 ...
 */
export function buildToolPolicy(agentDef, availableTools = [], basePolicy = {}) {
  const { tools, disallowedTools } = agentDef || {};
  const out = {};

  // Start from base policy (e.g. profile.defaultPolicy). Tools not listed
  // there fall back to "ask".
  for (const tool of availableTools) {
    out[tool] = basePolicy[tool] ?? "ask";
  }

  // Agent def whitelist: explicit allow overrides base.
