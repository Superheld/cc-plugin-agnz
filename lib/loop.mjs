...
import { recordEvent } from "./run-log.mjs";
...
import {
  SANDBOX_FRAMING, AVAILABLE_TOOLS, DENIED_TOOLS, SKILLS_HEADER, TURN_BUDGET_NOTICE,
  promptFingerprint,
} from "./prompts.mjs";
import { recoverTextualToolCalls, TEXTUAL_TOOL_CALL_NUDGE } from "./tool-call-recovery.mjs";
import { shouldCompact, performCompaction } from "./compaction.mjs";
import {
  mergeRange, coveredBy, fullRange, readRange, fmtRange,
  trackFocus, focusVerdict, focusNudge, focusQuestion,
} from "./repetition.mjs";
...
// Subdirectory context tracking (ADR 0012 phase 1) lives on the thread meta:
// `visitedDirs` is the "already seen" set; a subdir seen for the first time is
// queued in `pendingDirMds` so its CLAUDE.md is injected ONCE into history at
// the next turn boundary. Persisted (not process-local) so a resume in a fresh
// runner process cannot re-inject a dir the thread has already seen.
...
export async function runThread(ctx) {
  ...
  const maxTurns = resolveMaxTurns(thread, profile);
  ...
  // Bounded corrective retries for text-leaked tool calls (per run) — a model
  // that keeps missing the format should end the run, not loop on nudges.
  const MAX_TEXTUAL_NUDGES = 2;
  let textualNudges = 0;
  // One failed compaction disables further attempts this run — retrying a
  // broken summarize call every turn would burn the exact tokens it saves.
  ...
  // Stamp a live summary for the duration of the run: the previous pause's
  // summary ("reached turn limit", "needs approval: …") stays visible in
  // list/hook output otherwise and reads as CURRENT state while the thread is
  // running (dogfooding find, 2026-07-22). Overwritten again at every
  // pause/finish with the actual outcome.
  const workingSummary =
    userMessage != null ? `working: ${String(userMessage).slice(0, 120)}` : "working: resuming";
  await threadMgr.setStatus(thread.id, ThreadStatus.RUNNING, { pending: null, error: null, summary: workingSummary });
  ...
  // reuse its KV cache across turns. Persisted as its own write-once file
  // (ADR 0017) — inside the meta it was ~75 % of every meta rewrite. A legacy
  // thread that still carries the snapshot in its meta keeps using it.
  if (!thread.systemPromptSnapshot) {
    const existing = await threadMgr.readSystemPrompt(thread.id);
    const snapshot = existing ?? (await renderSystemPrompt({ thread, profile, registry, pluginRoot }));
    if (existing == null) {
      await threadMgr.writeSystemPrompt(thread.id, snapshot);
      // Stamp which agnz wrote it. The snapshot is write-once, so a thread
      // outlives prompt fixes; `show` compares this against the running
      // version to make that drift visible instead of silent.
      const promptVersion = await readPluginVersion(pluginRoot);
      const promptTemplates = promptFingerprint();
      await threadMgr.updateThread(thread.id, { promptVersion, promptTemplates });
      Object.assign(thread, { promptVersion, promptTemplates });
    }
    Object.assign(thread, { systemPromptSnapshot: snapshot });
  }

  // ADR 0020: this run's identity. Every log entry carries it, which is what
  // makes "what happened in the run that failed" answerable at all — the old
  // trace was thread-scoped and thread_end in fact fired per run.
  const runId = ctx.runId || randomUUID();
  threadMgr.setRunContext?.({ threadId: thread.id, runId, turn: null });
  await threadMgr.log?.(thread.id, "run_start", {
    pid: process.pid,
    trigger: resumeInput ? "resume" : userMessage != null ? "message" : "drain",
    cwd: thread.cwd,
    endpoint: profile.baseUrl ?? null,
    profile: profile.name ?? null,
    profileOrigin: profile.origin ?? null,
    model: profile.model ?? null,
    agentDef: thread.agentDef?.name ?? null,
    maxTurns,
    promptVersion: thread.promptVersion ?? null,
    promptTemplates: thread.promptTemplates ?? null,
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      threadMgr.setRunContext?.({ threadId: thread.id, runId, turn });
      ...
      // Context compaction (context-diet 3/3): when the last observed prompt
      // size crossed the profile's window threshold, pay one deliberate cache
      // reset — summarize the session, append the marker, clear the knowledge
      // state. Checked here (clean boundary: leftovers drained, nothing
      // pending) so the marker lands on a template-valid seam. Null out
      // lastPromptTokens afterwards: it described the pre-compaction context,
      // and keeping it would re-trigger every turn on servers that omit usage.
      if (!compactDisabled && shouldCompact({ lastPromptTokens, profile })) {
        const compacted = await performCompaction({ thread, threadMgr, profile, chatFn, turn, signal });
        if (compacted) lastPromptTokens = null;
        else compactDisabled = true;
      }
      ...
      await drainTopOfTurnContext({ thread, threadMgr, turn, maxTurns });
      ...
      if (firstEverRun && turn === 0) {
        recordEvent(thread, {
          ...
          // How this run was actually wired up. The trace recorded the model
          // name but never the endpoint, so a host-unreachable failure could
          // not be traced back to an address — and with several profiles on
          // one host, the model name does not identify it. No apiKey here:
          // traces are plain files inside the project.
          endpoint: profile.baseUrl ?? null,
          invocation: {
            cwd: thread.cwd,
            profile: profile.name ?? null,
        ...
        recordEvent(thread, { type: "turn_start", turn });
      }

      // ADR 0020 section 3: the request names the seq range it sent instead of
      // copying the conversation into itself. messages[0] is the frozen system
      // prompt (referenced by digest, stored once in <id>.system.txt), so the
      // rest is exactly the message tail — after a compaction, the tail from
      // the marker on.
      const sentRange = await threadMgr.logRangeForLastMessages?.(thread.id, messages.length - 1);
      await threadMgr.log?.(thread.id, "api_request", {
        model: profile.model ?? null,
        endpoint: profile.baseUrl ?? null,
        prefix: thread.promptTemplates ?? null,
        messages: sentRange,
        toolCount: tools.length,
        params: {
          temperature: thread.agentDef?.temperature ?? profile.temperature ?? null,
      ...
      try {
        ...
        // A failed call used to leave NO llm_call event at all: the trace went
        // turn_start → (nothing) → thread_end, so how long the call ran, which
        // endpoint it addressed and why it failed were all unrecoverable
        // afterwards. Record it as the same event type with outcome:"error"
        // before rethrowing, so success and failure sit on one timeline.
        await threadMgr.log?.(thread.id, "api_error", {
      ...
      const { message, finishReason, usage } = llmResult;
      ...
      await threadMgr.log?.(thread.id, "api_response", {
        latencyMs: Date.now() - llmStart,
      ...
      // A message without tool_calls is *usually* a final answer — but local
      // models sometimes emit their call in the model-native text syntax
      // (e.g. Mistral's `Name[ARGS]{...}`) that the server's parser missed.
      // Recover deterministically when possible; nudge (bounded) when it
      // only looks like an attempt. Recovery must happen BEFORE the
      // assistant message is persisted: the tool results that follow need a
      // matching tool_calls entry or strict templates reject the history.
      let toolCalls = message.tool_calls || [];
      if (toolCalls.length === 0 && typeof message.content === "string") {
        const rec = recoverTextualToolCalls(message.content, registry.list().map((t) => t.name));
        if (rec && rec.toolCalls.length > 0) {
          toolCalls = rec.toolCalls;
          stats.repairs += 1;
          recordEvent(thread, {
            type: "repair", turn, kind: "textual_tool_call", recovered: true,
            tools: toolCalls.map((c) => c.function.name),
          });
        } else if (rec && rec.attempted && textualNudges < MAX_TEXTUAL_NUDGES) {
          textualNudges += 1;
          stats.repairs += 1;
          recordEvent(thread, { type: "repair", turn, kind: "textual_tool_call", recovered: false });
          await threadMgr.appendMessage(thread.id, {
            role: "assistant",
            content: message.content ?? null,
          });
          await threadMgr.appendMessage(thread.id, { role: "user", content: TEXTUAL_TOOL_CALL_NUDGE });
          continue;
        }
      }
      ...
      if (toolCalls.length === 0) {
        ...
        await threadMgr.log?.(thread.id, "run_end", { reason: "final", totals: { ...stats } });
    recordEvent(thread, { type: "thread_end", reason: "final", turns: turn + 1, totals: { ...stats } });
    ...
    await threadMgr.log?.(thread.id, "run_end", { reason: "max_turns", totals: { ...stats } });
    recordEvent(thread, { type: "thread_end", reason: "max_turns", turns: maxTurns, totals: { ...stats } });
    ...
    if (signal?.aborted) {
      await threadMgr.log?.(thread.id, "run_end", { reason: "stopped", totals: { ...stats } });
    recordEvent(thread, { type: "thread_end", reason: "stopped", totals: { ...stats } });
    ...
    await threadMgr.log?.(thread.id, "run_end", { reason: "error", totals: { ...stats } });
    recordEvent(thread, { type: "thread_end", reason: "error", error: err.message, totals: { ...stats } });
    throw err;
  }
}
 ...
 */
// Remaining-turn counts at which the agent is told where it stands. A few
// marks, not every turn: the notice has to read as a signal, and injecting it
// on all 40 turns would train the model to skip it.
const TURN_BUDGET_MARKS = [10, 5, 2];

async function drainTopOfTurnContext({ thread, threadMgr, turn, maxTurns }) {
  const sections = [];

  // (1) one-time CLAUDE.md for newly-visited subdirs (queue lives on the meta)
  const pend = Array.isArray(thread.pendingDirMds) ? thread.pendingDirMds : [];
  if (pend.length > 0) {
    const dirs = pend;
    await threadMgr.updateThread(thread.id, { pendingDirMds: [] });
    thread.pendingDirMds = [];
    const blocks = [];
    for (const dir of dirs) {
      const md = await readProjectMd(dir);
      if (md) blocks.push(md.content);
    }
    if (blocks.length > 0) {
      sections.push(
  ...
  // (3) turn budget. The prefix announces the total once on turn 0; without
  // this the agent never learns its position and cannot obey the prompt's own
  // "finish the most valuable part" rule before hitting the wall.
  if (Number.isInteger(turn) && Number.isInteger(maxTurns)) {
    const remaining = maxTurns - turn;
    if (TURN_BUDGET_MARKS.includes(remaining)) {
      sections.push(
        TURN_BUDGET_NOTICE.replace("{remaining}", String(remaining)).replace(
          "{maxTurns}",
          String(maxTurns),
        ),
      );
...
async function buildMessages({ thread, threadMgr, profile, registry, pluginRoot }) {
  ...
  // A compaction marker (context-diet 3/3) restarts the wire payload: send
  // only from the newest marker on. Everything before it stays on disk.
  const history = await threadMgr.readMessages(thread.id);
  let start = 0;
  for (let i = history.length - 1; i >= 0; i--) {
...
async function renderSystemPrompt({ thread, profile, registry, pluginRoot }) {
  // Every layer below is built for every thread. A thread without an agent def
  // used to return here with nothing but its role text — no sandbox framing, no
  // tool policy, no workflow rules — while the harness went on enforcing those
  // rules anyway, so the agent could be blocked by a rule it was never told.
  // A custom systemPrompt now takes the same slot as an agent body: it adds a
  // role, it does not replace the harness contract.
  const effectivePolicy = buildToolPolicy(
    thread.agentDef,
    registry.list().map((t) => t.name),
  );

  const parts = [defaultSystemPrompt(thread, profile)];

  // Startup CLAUDE.md/AGENTS.md: cwd only. Subdirectory context is injected
  // once into history as the agent works there, never baked into this frozen
  // prefix. The header tells the agent the conventions are already loaded so
  // it does not burn a turn re-reading the file.
  const claudeMds = await collectClaudeMds(thread.cwd);
  if (claudeMds.length > 0) {
    parts.push(
      "## Project conventions (already loaded — do not re-read these files)\n\n" +
        claudeMds.map(({ content }) => content).join("\n\n---\n\n"),
    );
  }
  ...
  // The role layer, last so it can lean on everything above. Agent body lives
  // in the system prompt (putting it in a user message would create consecutive
  // user turns on turn 0 and break strict-alternation model templates like
  // Mistral's). Without an agent def a configured systemPrompt fills the slot.
  const role =
    thread.agentDef?.prompt ||
...
// One resolution path for the turn budget, shared by the loop's enforcement
// and the prompt's announcement — the number the agent is told must be the
// number the loop enforces.
function resolveMaxTurns(thread, profile) {
  return thread.agentDef?.maxTurns ?? profile?.maxTurns ?? DEFAULT_MAX_TURNS;
}

// All placeholders are static per thread (name and turn budget never change
// after creation), so the rendered result stays safe to freeze (ADR 0012).
function defaultSystemPrompt(thread, profile) {
  return SANDBOX_FRAMING.replace("{cwd}", thread.cwd)
    .replaceAll("{agentName}", agentNameFor(thread))
    .replace("{maxTurns}", String(resolveMaxTurns(thread, profile)));
}
...
async function buildSkillCatalog(cwd, skillNames, pluginRoot) {
  ...
  return [...catalog.values()]
    ...
    .map((sk) => {
      // Collapse internal whitespace: a folded YAML block scalar maps paragraph
      // breaks to "\n", and this catalog is the one description consumer that
      // joins its lines raw — an embedded newline would inject an unframed line
      // into the sub-agent system prompt. Every other consumer already
      // collapses; do it here so a multi-paragraph description stays one line.
      const desc = sk.description ? String(sk.description).replace(/\s+/g, " ").trim() : "";
...
function checkWorkflowDiscipline({ name, args, thread, sandbox }) {
  ...
  if (name === "Write" || name === "Edit") {
    ...
    // Freshness gate, Write only (context-diet 2/3): a Write replaces the whole
    // file with no anchor to catch staleness, so "read at some point" is not
    // enough — the file must be unchanged since the agent's last look. Edit is
    // deliberately exempt: its anchor match against the on-disk bytes already
    // fails loudly on a stale view.
    if (name === "Write") {
      const stamp = thread.fileStamps?.[abs];
      if (stamp) {
        let st = null;
        try { st = statSync(abs); } catch { return null; }
        if (st && (st.mtimeMs !== stamp.mtimeMs || st.size !== stamp.size)) {
          return `Workflow: '${args.path}' has changed on disk since you last read it — your copy is stale. Read it again (or the relevant slice), then retry the Write.`;
        }
      }
    }
    return null;
...
/**
 * Context-diet 2/3: dedup a full re-read of a file whose entire, unchanged
 * content is already in the agent's context. Only fires when the previous
 * knowledge was itself full (a slice reader must be allowed to widen to the
 * whole file) and mtime+size still match. Returns the substitute tool result,
 * or null to let the Read run. The compaction reset (3/3) clears fileStamps,
 * so this can never claim "already in your context" across a compaction.
 */
function checkReadDedup({ args, thread, sandbox }) {
  let abs;
  try { abs = sandbox.resolvePath(args?.path); } catch { return null; }
  const stamp = thread.fileStamps?.[abs];
  ...
  let st = null;
  try { st = statSync(abs); } catch { return null; }
  if (st.mtimeMs !== stamp.mtimeMs || st.size !== stamp.size) return null;

  const isSlice = args && (args.start_line != null || args.end_line != null);
  if (!isSlice) {
    if (stamp.full !== true) return null;
    return `'${args.path}' is unchanged since your last read — its full content is already in your context above (not re-sending it). If you need a specific part fresh anyway, Read with start_line/end_line.`;
  }

  // Coverage dedup: the requested slice sits inside what the agent has
  // already been shown. Widening still passes (partial overlap sends the
  // whole slice) — only a strictly redundant window is answered with a
  // pointer, which is what turns a circling re-read into a cheap no-op.
  const range = readRange(args);
  if (!coveredBy(stamp.ranges, ...range)) return null;
  return `'${args.path}' lines ${fmtRange(range)} are unchanged and already in your context above (not re-sending them). You have read this region before — if it did not answer your question, the answer is not there: change approach rather than re-reading it.`;
}
...
async function recordKnownFile({ tool, args, sandbox, thread, threadMgr }) {
  ...
  const known = Array.isArray(thread.knownFiles) ? thread.knownFiles : [];
  const patch = {};
  if (!known.includes(abs)) patch.knownFiles = [...known, abs];

  // Freshness stamp (context-diet 2/3): what the agent's context holds about
  // this file. `full` = the agent has seen the ENTIRE current content — a
  // no-slice Read, or a Write (it authored the bytes). An Edit keeps the prior
  // flag (it changed a region it had read; total knowledge is unchanged) but
  // must refresh mtime/size, or the agent's own edit would trip the Write
  // freshness gate as a foreign change.
  let st = null;
  try { st = statSync(abs); } catch { /* deleted mid-run: drop the stamp below */ }
  const stamps = { ...(thread.fileStamps || {}) };
  if (!st) {
    delete stamps[abs];
  } else {
    const prev = stamps[abs];
    const full =
      tool.name === "Write" ? true :
      tool.name === "Edit" ? prev?.full === true :
      !(args.start_line != null || args.end_line != null);
    // Line ranges the agent has actually seen, for the coverage dedup.
    // An Edit DROPS them: it can insert or remove lines, so every recorded
    // range may now point at different content. Claiming "you already read
    // 186-189" after a shift would be a lie, and a cheap wrong answer is
    // worse than an honest re-read.
    let ranges;
    if (tool.name === "Write") ranges = fullRange();
    else if (tool.name === "Edit") ranges = [];
    else if (full) ranges = fullRange();
    else ranges = mergeRange(prev?.ranges, ...readRange(args));
    stamps[abs] = { mtimeMs: st.mtimeMs, size: st.size, full, ranges };
  }
  patch.fileStamps = stamps;

  await threadMgr.updateThread(thread.id, patch);
  Object.assign(thread, patch);
}

// Compact "what is it working on" label for a tool_call trace event: the
// path for file tools, a command/pattern prefix otherwise. Powers the
// last-activity line in the lead block and `agnz show`/`list`/`wait`.
// Returns undefined when there is nothing path-ish (JSON.stringify drops
// undefined values, so the trace entry simply has no `target` field).
function toolTarget(name, args) {
  if (!args || typeof args !== "object") return undefined;
  if (typeof args.path === "string" && args.path) return args.path.slice(0, 120);
  if (name === "Bash" && typeof args.command === "string" && args.command) return args.command.slice(0, 80);
  if (name === "Grep" && typeof args.pattern === "string" && args.pattern) return args.pattern.slice(0, 80);
  if (name === "Skill" && typeof args.name === "string" && args.name) return args.name;
  return undefined;
}
...
async function dispatchToolCall({ call, sandbox, registry, thread, threadMgr, pluginRoot, turn, stats, getCard }) {
  ...
  if (!tool) {
    ...
    recordEvent(thread, { type: "tool_call", turn: turn ?? null, name, outcome: "error" });
    return { status: "ok" };
  }
  ...
  try {
    ...
    try {
      ...
      recordEvent(thread, { type: "repair", turn: turn ?? null, tool: name, recovered: true });
    } catch (err) {
      recordEvent(thread, { type: "repair", turn: turn ?? null, tool: name, recovered: false });
  ...
  if (name === "AskUser") {
    ...
    recordEvent(thread, { type: "pause", turn: turn ?? null, kind: "question" });
    publish(thread.cwd, {
  ...
  // Repetition guard: an agent circling one file burns the whole turn budget
  // and reports "turn limit" (6 of 21 field threads). Runs before the
  // discipline check so a blocked retry loop is caught too.
  if (typeof args?.path === "string" && args.path) {
    let focusAbs = null;
    try { focusAbs = sandbox.resolvePath(args.path); } catch { /* unresolvable: skip */ }
    if (focusAbs) {
      const focus = trackFocus(thread.focus, focusAbs);
      const verdict = focusVerdict(focus);

      if (verdict === "nudge") {
        await threadMgr.updateThread(thread.id, { focus: { ...focus, nudged: true } });
        Object.assign(thread, { focus: { ...focus, nudged: true } });
        await appendToolResult(threadMgr, thread.id, toolCallId, focusNudge(focus, args.path));
        recordEvent(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "blocked", reason: "repetition" });
        return { status: "ok" };
      }

      if (verdict === "ask") {
        // Reset the streak: the lead's answer is the fresh start, and without
        // this the very next call would pause again.
        await threadMgr.updateThread(thread.id, { focus: { path: focusAbs, count: 0, nudged: false } });
        const question = focusQuestion(focus, args.path);
        const pending = { toolCallId, kind: "question", question, options: null, context: null };
        await threadMgr.setStatus(thread.id, ThreadStatus.AWAITING_INPUT, { pending, summary: `stuck on ${args.path} (${focus.count} calls), asked lead`, ...(getCard ? { card: getCard() } : {}) });
        recordEvent(thread, { type: "pause", turn: turn ?? null, kind: "question", reason: "repetition" });
        publish(thread.cwd, {
          from: agentNameFor(thread), to: "parent", kind: "question", urgent: true,
          text: `[${thread.id}] Agent is stuck and paused for direction.\nQuestion: ${question}`,
        }).catch(() => {});
        return { status: "awaiting_input", pending };
      }

      // The write is NOT optional, tempting as it looks on the hot path: the
      // turn loop reloads the thread from disk every turn (see the
      // getThread refresh above), so an in-memory-only counter is wiped
      // between turns and — with one tool call per turn being the norm —
      // would never climb past 1.
      await threadMgr.updateThread(thread.id, { focus });
      Object.assign(thread, { focus });
    }
  }
  ...
  if (correction) {
    ...
    recordEvent(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "blocked" });
    return { status: "ok" };
  }

  // Context-diet: answer a redundant full re-read without re-sending the
  // content. Not a violation (traced "deduped", not "blocked") — the agent
  // asked a legitimate question whose answer is already in its context.
  if (name === "Read") {
    const dedup = checkReadDedup({ args, thread, sandbox });
    if (dedup) {
      await appendToolResult(threadMgr, thread.id, toolCallId, dedup);
      recordEvent(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "deduped" });
      return { status: "ok" };
    }
  }
  ...
  if (name === "Bash" && decision === Decision.ASK) {
    ...
    if (sc.sessionDeny?.includes(command)) {
      ...
      recordEvent(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "denied" });
      return { status: "ok" };
    }
  ...
  if (decision === Decision.ASK) {
    ...
    recordEvent(thread, { type: "pause", turn: turn ?? null, kind: "approval", tool: name });
...
async function runToolAndAppend({ tool, args, toolCallId, sandbox, thread, threadMgr, pluginRoot, turn, stats }) {
  ...
  try {
    ...
    recordEvent(thread, { type: "tool_call", turn: turn ?? null, name: tool.name, target: toolTarget(tool.name, args), latencyMs: Date.now() - started, outcome: "ok" });
    ...
    if (args.path && (tool.name === "Read" || tool.name === "Write" || tool.name === "Edit")) {
      ...
      if (dir.startsWith(thread.cwd + "/")) {
        const seen = Array.isArray(thread.visitedDirs) ? thread.visitedDirs : [];
        if (!seen.includes(dir)) {
          const pend = Array.isArray(thread.pendingDirMds) ? thread.pendingDirMds : [];
          const patch = { visitedDirs: [...seen, dir], pendingDirMds: [...pend, dir] };
          await threadMgr.updateThread(thread.id, patch);
          Object.assign(thread, patch);
        }
      }
    ...
    if (stats) stats.toolCalls += 1;
    recordEvent(thread, { type: "tool_call", turn: turn ?? null, name: tool.name, target: toolTarget(tool.name, args), latencyMs: Date.now() - started, outcome: "error" });
    return { status: "ok" };
  }
}
...
// on every pause/finish. The parent (and `agnz show`) read it to see
// reusable context without opening the transcript. Cheap: derived from the
// agent's own latest output, no extra LLM call.
/**
 * First substantive line of a final reply — this becomes the thread summary
 * the lead reads in `show` and the hook block.
 *
 * Taking the literal first line was wrong in the field: a report opening with
 * a markdown heading stored "## Summary" (dash-data), and one opening with an
 * announcement stored "…Let me create a summary of what I've built:"
 * (dash-ui). Both threw away the actual report. Headings, rules and lines
 * that merely introduce what follows are skipped; if nothing substantive is
 * found the original first line still wins, so a terse reply is never lost.
 */
function summarize(text) {
  ...
  const substantive = lines.find(
    (l) =>
      !/^#{1,6}\s/.test(l) &&   // markdown heading
      !/^([-*_=])\1{2,}$/.test(l) && // horizontal rule
      !/:$/.test(l),            // "Summary:", "Let me …:" — introduces, says nothing
  );
  return (substantive || lines[0]).slice(0, 140) || null;
}
...
async function collectClaudeMds(cwd, extraDirs = []) {
  ...
  // cwd itself first
  seen.add(cwd);
  const cwdMd = await readProjectMd(cwd);
  if (cwdMd) results.push(cwdMd);
...
/**
 * Read a directory's project-context file: CLAUDE.md first, else AGENTS.md
 * (the vendor-neutral convention), so agnz drops into repos that standardise
 * on either. First hit wins per directory: many repos symlink one to the
 * other, and injecting both would duplicate the content.
 */
async function readProjectMd(dir) {
...
async function resolvePending({ thread, threadMgr, sandbox, registry, resume, pluginRoot, stats }) {
  ...
  // Record the decision only when the caller asked for it (`--persist`).
  // Without the flag an approval resolves exactly one pending call — the
  // documented contract, silently lost in the policy-simplification refactor
  // (137167a) which made every approval persist and the flag a no-op.
  // Scope of a persisted decision: Bash commands go into meta sessionCommands
  // (per thread — survives later runs); other tools into the sandbox's
  // in-memory record (rest of the current run).
  if (resume.persist === true && pending.name === "Bash") {
  ...
  if (decision === Decision.DENY) {
    ...
    recordEvent(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), outcome: "denied" });
    await threadMgr.setStatus(thread.id, ThreadStatus.RUNNING, { pending: null });
    return { status: "ok" };
  }
  ...
  try {
    ...
    if (stats) stats.toolCalls += 1;
    recordEvent(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), latencyMs: Date.now() - started, outcome: "ok" });
    ...
    if (stats) stats.toolCalls += 1;
    recordEvent(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), latencyMs: Date.now() - started, outcome: "error" });
  }
  await threadMgr.setStatus(thread.id, ThreadStatus.RUNNING, { pending: null });
  return { status: "ok" };
}
