...
async function buildMessages({ thread, threadMgr, profile, registry, pluginRoot }) {
  ...
  if (thread.agentDef) {
    ...
    // Skills injection:
    // - explicit list in agentDef.skills → eager: full content injected directly.
    //   Small local models don't reliably call Skill({action:"load"}), so we
    //   inject everything upfront when the agent author explicitly opted in.
    // - no list (null) → catalog only: name+description, agent loads on demand.
    const skillFilter = Array.isArray(thread.agentDef.skills) ? thread.agentDef.skills : null;
    if (skillFilter !== null) {
      const skillBlocks = await buildSkillBlocks(thread.cwd, skillFilter, pluginRoot);
      if (skillBlocks.length > 0) parts.push(skillBlocks.join("\n\n---\n\n"));
    } else {
      const catalog = await buildSkillCatalog(thread.cwd, null, pluginRoot);
 ...
 */
async function buildSkillBlocks(cwd, skillNames, pluginRoot) {
  const { readdir } = await import('node:fs/promises');
  ...
  // Map skillName → { description, body } (project wins on clash)
  const found = new Map();
  for (const root of roots) {
    let entries;
    ...
    for (const entry of entries) {
      ...
      const skillPath = resolve(root, entry.name, 'SKILL.md');
      let source;
      try { source = await readFile(skillPath, 'utf8'); } catch { continue; }
      const lines = source.replace(/^﻿/, "").replace(/\r\n?/g, "\n").split("\n");
      let skillName = entry.name;
      let description = "";
      let inFrontmatter = false;
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].trim() === "---") {
          ...
          else { frontmatterDone = true; bodyStart = i + 1; break; }
        }
        if (inFrontmatter) {
          const mName = lines[i].match(/^name\s*:\s*(.+)$/);
          if (mName) skillName = mName[1].trim();
          const mDesc = lines[i].match(/^description\s*:\s*(.+)$/);
          if (mDesc) description = mDesc[1].trim().replace(/^["']|["']$/g, "");
        }
      }
      const body = frontmatterDone ? lines.slice(bodyStart).join("\n").trim() : source.trim();
      found.set(skillName, { description, body });
    }
  }
  ...
  return skillNames
    ...
    .map((n) => {
      const { description, body } = found.get(n);
      const header = description ? `## Skill: ${n}\n${description}` : `## Skill: ${n}`;
      return `${header}\n\n${body}`;
    });
}
