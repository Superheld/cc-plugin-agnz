...
async function buildMessages({ thread, threadMgr, profile }) {
  ...
  // Agent body becomes the initial user message (ADR 0003)
  if (thread.agentDef?.body) {
    messages.push({ role: "user", content: thread.agentDef.body });
  }
...
async function resolvePending({ thread, threadMgr, sandbox, registry, resume }) {
  ...
  // Bash commands: stored in thread meta's sessionCommands.
  // Other tools: recorded to sandbox policy for this session.
