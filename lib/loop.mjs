  ...
  if (name === "Bash" && decision === Decision.ASK) {
    ...
    if (resolved === "deny") {
      await appendToolResult(
        ...
        thread.id,
        toolCallId,
        `Error: command '${command}' is denied by command policy.`,
      );
      return { status: "ok" };
    }
  }
...
async function resolvePending({ thread, threadMgr, sandbox, registry, resume }) {
  ...
  if (pending.name === "Bash") {
    ...
    if (command) {
      const store = createWorkspaceStore(thread.cwd);
      const agentName = agentNameFor(thread);
      const list = decision === Decision.ALLOW ? "allow" : "deny";
      await store.addToAgentCommands(agentName, list, command);
    }
  } else if (resume.persist) {
