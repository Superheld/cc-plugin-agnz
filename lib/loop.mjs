...
async function collectClaudeMds(cwd, extraDirs = []) {
  ...
  // Upward scan: home → cwd (reversed so we push outer dirs first)
  const upward = [];
  let d = cwd;
  while (true) {
    upward.unshift(d);
    if (d === home || dirname(d) === d) break;
    d = dirname(d);
  }
  ...
  // Visited subdirs: for each tracked dir scan from cwd inward
  for (const extraDir of extraDirs) {
    if (!extraDir.startsWith(cwd) || extraDir === cwd) continue;
    // Build chain from just-below-cwd down to extraDir
    const chain = [];
    let cur = extraDir;
    while (cur.startsWith(cwd) && cur !== cwd) {
      chain.unshift(cur);
      cur = dirname(cur);
    }
