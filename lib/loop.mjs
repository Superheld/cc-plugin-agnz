...
function defaultSystemPrompt(thread) {
  ...
  // ADR 0005: inject the skills hint when the agent def declares a skills
  // list. Without the hint the agent may not think to use use_skill at all.
  const skills = thread.agentDef?.skills;
  if (Array.isArray(skills) && skills.length > 0) {
    lines.push(
      "",
      "Skills: you have project-specific instruction sets available via use_skill.",
      "Call use_skill({action:'list'}) to see what is available, then use_skill({action:'load', name:'...'}) to pull in the content of a relevant skill before starting the task.",
    );
  }
...
async function dispatchToolCall({ call, sandbox, registry, thread, threadMgr }) {
  ...
  // Special-case: ask_user is intercepted before policy / run.
  if (name === "ask_user") {
