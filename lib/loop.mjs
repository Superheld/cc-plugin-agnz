 ...
 * ADR 0006: agents get a catalog at startup and Read the full SKILL.md
 * themselves when they need it — same progressive-disclosure model as
 * the main Claude Code session. Unknown skill names are skipped silently.
 */
async function buildSkillCatalog(cwd, skillNames) {
  ...
  for (const name of skillNames) {
    ...
    // Extract description from frontmatter (one-liner or first non-empty line after key).
    ...
    entries.push(`- ${name}: ${description || "(no description)"} — ${skillPath}`);
  }
  return entries;
}
