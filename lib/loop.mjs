...
async function buildMessages({ thread, threadMgr, profile }) {
  ...
  if (thread.agentDef) {
    ...
    const toolNote = buildToolRestrictionsNote(thread.policy);
    ...
    // Also show tool restrictions for non-agent threads
    const toolNote = buildToolRestrictionsNote(thread.policy);
    if (toolNote) system = system + "\n\n" + toolNote;
  }
 ...
 */
async function resolveBashCommand({ command, thread, threadMgr, sandbox }) {
  const store = createWorkspaceStore(thread.cwd);
  const agentName = agentNameFor(thread);
  const lists = await store.getAgentCommands(agentName);
  const sessionCommands = thread.sessionCommands || { sessionAllow: [], sessionDeny: [] };

  // Check workspace.json permanent deny list first
  if (lists.deny.includes(command)) return "deny";

  // Check workspace.json permanent allow list
  if (lists.allow.includes(command)) return "allow";

  // Check session deny list (from agent_approve)
  if (sessionCommands.sessionDeny.includes(command)) return "deny";

  // Check session allow list (from agent_approve)
  if (sessionCommands.sessionAllow.includes(command)) return "allow";
...
async function dispatchToolCall({ call, sandbox, registry, thread, threadMgr }) {
  ...
  if (name === "Bash" && decision === Decision.ASK) {
    ...
    const resolved = await resolveBashCommand({
      command,
      thread,
      threadMgr,
      sandbox,
    ...
    if (resolved === "deny") {
      await appendToolResult(
        ...
        thread.id,
        toolCallId,
        `Error: command '${command}' is denied.`,
      );
      return { status: "ok" };
    }
    // resolved === "ask" → fall through to pause
  }
...
async function resolvePending({ thread, threadMgr, sandbox, registry, resume }) {
  ...
  // persist=true on Bash: record at the tool level (allow/deny all Bash for this
  // thread) so the agent doesn't have to ask again for every distinct command.
  // Without persist, only the specific command is stored in sessionCommands.
  if (pending.name === "Bash" && !resume.persist) {
