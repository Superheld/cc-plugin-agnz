...
export async function runThread(ctx) {
  ...
  if (!thread.systemPromptSnapshot) {
    ...
    if (existing == null) await threadMgr.writeSystemPrompt(thread.id, snapshot);
...
async function renderSystemPrompt({ thread, profile, registry, pluginRoot }) {
  if (!thread.agentDef) {
    return profile.systemPrompt || thread.systemPrompt || defaultSystemPrompt(thread, profile);
  }
