...
export async function runThread(ctx) {
  ...
  // Write the static debug snapshot to meta once per thread (not per turn).
  // tools never change during a thread; re-check only on first run.
  if (!thread.debugTools) {
    const toolSnapshot = registry.list().map((t) => ({ name: t.name, description: t.description }));
    await threadMgr.updateThread(thread.id, { debugTools: toolSnapshot });
    Object.assign(thread, { debugTools: toolSnapshot });
  }
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      // Append a turn snapshot to <thread-id>.turns.jsonl so the system
      // prompt history is fully inspectable. The file sits next to the
      // .meta.json in the threads directory. Failures are silent — debug
      // logging must never crash the agent loop.
      appendDebugTurn(thread, turn, messages[0].content).catch(() => {});
...
/**
 * Append a per-turn debug snapshot to <thread-id>.turns.jsonl.
 * Records the system prompt as sent to the LLM so we can trace exactly what
 * the agent knew at each API call. Kept separate from the messages transcript
 * to avoid polluting the conversation history with debug data.
 */
async function appendDebugTurn(thread, turn, systemPrompt) {
  const dir = resolve(thread.cwd, ".claude", "agnz", "threads");
  const file = resolve(dir, `${thread.id}.turns.jsonl`);
  const entry = { ts: Date.now(), turn, systemPrompt };
