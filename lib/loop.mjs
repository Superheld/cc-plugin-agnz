    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (toolCalls.length === 0 && typeof message.content === "string") {
        ...
        if (rec && rec.toolCalls.length > 0) {
          ...
          textualNudges += 1;
          stats.repairs += 1;
          appendTrace(thread, { type: "repair", turn, kind: "textual_tool_call", recovered: false });
...
async function dispatchToolCall({ call, sandbox, registry, thread, threadMgr, pluginRoot, turn, stats, getCard }) {
  ...
  if (typeof args?.path === "string" && args.path) {
    ...
    if (focusAbs) {
      ...
      if (verdict === "nudge") {
        ...
        appendTrace(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "blocked", reason: "repetition" });
        return { status: "ok" };
      }
      ...
      if (verdict === "ask") {
        ...
        appendTrace(thread, { type: "pause", turn: turn ?? null, kind: "question", reason: "repetition" });
        publish(thread.cwd, {
          from: agentNameFor(thread), to: "parent", kind: "question", urgent: true,
  ...
  if (correction) {
    ...
    appendTrace(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "blocked" });
    return { status: "ok" };
  }
  ...
  if (name === "Read") {
    ...
    if (dedup) {
      ...
      appendTrace(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "deduped" });
      return { status: "ok" };
    }
  }
  ...
  if (name === "Bash" && decision === Decision.ASK) {
    ...
    if (sc.sessionDeny?.includes(command)) {
      ...
      appendTrace(thread, { type: "tool_call", turn: turn ?? null, name, target: toolTarget(name, args), outcome: "denied" });
      return { status: "ok" };
    }
...
async function runToolAndAppend({ tool, args, toolCallId, sandbox, thread, threadMgr, pluginRoot, turn, stats }) {
  ...
  try {
    ...
    appendTrace(thread, { type: "tool_call", turn: turn ?? null, name: tool.name, target: toolTarget(tool.name, args), latencyMs: Date.now() - started, outcome: "ok" });
    ...
    if (stats) stats.toolCalls += 1;
    appendTrace(thread, { type: "tool_call", turn: turn ?? null, name: tool.name, target: toolTarget(tool.name, args), latencyMs: Date.now() - started, outcome: "error" });
    return { status: "ok" };
  }
}
...
async function resolvePending({ thread, threadMgr, sandbox, registry, resume, pluginRoot, stats }) {
  ...
  if (decision === Decision.DENY) {
    ...
    appendTrace(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), outcome: "denied" });
    await threadMgr.setStatus(thread.id, ThreadStatus.RUNNING, { pending: null });
    return { status: "ok" };
  }
  ...
  try {
    ...
    if (stats) stats.toolCalls += 1;
    appendTrace(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), latencyMs: Date.now() - started, outcome: "ok" });
    ...
    if (stats) stats.toolCalls += 1;
    appendTrace(thread, { type: "tool_call", turn: null, name: pending.name, target: toolTarget(pending.name, pending.args), latencyMs: Date.now() - started, outcome: "error" });
  }
  await threadMgr.setStatus(thread.id, ThreadStatus.RUNNING, { pending: null });
  return { status: "ok" };
}
