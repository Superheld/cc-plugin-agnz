...
export function parseSkillMd(source, dirName) {
  ...
  if (i >= lines.length || lines[i].trim() !== "---") {
    return { name: dirName, description: "", audience: null, body: source.trim() };
  }
  ...
  while (i < lines.length && lines[i].trim() !== "---") {
    ...
    if (key === "audience" && value.trim()) audience = value.trim().toLowerCase();
  }
  if (i < lines.length && lines[i].trim() === "---") i++; // consume closing fence
  const body = lines.slice(i).join("\n").trim();
  return { name, description, audience, body };
}
...
export async function discoverSkills(cwd, pluginRoot) {
  ...
  for (const { dir, source } of roots) {
    ...
    for (const entry of entries) {
      ...
      // `audience: lead` keeps a skill out of the sub-agent's world entirely.
      // agnz ships its own orchestration docs as skills, and discovery reaches
      // the plugin root, so every sub-agent's frozen system prompt used to
      // list "agnz: delegate this to an agent" and "agnz-setup: configure
      // profiles" — telling a worker it can spawn workers and reconfigure the
      // plugin. Those skills are written FOR the lead and should read that
      // way; the fix is not to sanitise their prose but to stop handing them
      // to the wrong reader. Absent field = available, so nothing else moves.
      if (parsed.audience === "lead") continue;
