...
// The open writer per thread. Module-level because the loop records events from
// a dozen call sites, most of them helpers that were never given the writer —
// and because a second writer would allocate its own sequence numbers and tear
// the ordering. One process, one thread, one run (claimThread guarantees it),
// so there is exactly one correct writer at any moment.
...
export async function openThreadLog({ cwd, threadId, runId }) {
  ...
  activeWriters.set(threadId, writer);
  return writer;
}

/** The open writer for a thread, or null when nothing is running. */
export function writerFor(threadId) {
  return activeWriters.get(threadId) || null;
}

// Legacy trace shapes → log events. The loop's recording sites grew under the
// old two-file scheme, where `<id>.trace.jsonl` held timing and outcomes while
// `<id>.jsonl` held content. Their event names described that split; the log
// does not have it. Translating centrally migrates every site at once and keeps
// one writer owning the sequence.
//
// Dropped entirely, because the log already says it better:
//   thread_start / turn_start — run_start carries the invocation, and every
//                               entry carries its own turn
//   llm_call                  — api_request/api_response/api_error, which also
//                               separate "asked" from "answered"
//   thread_end                — run_end, which is honestly named: it always was
//                               a run ending, not a thread
const EVENT_MAP = {
  tool_call: "tool_exec",
  repair: "harness",
  compaction: "harness",
...
const DROPPED = new Set(["thread_start", "turn_start", "llm_call", "thread_end"]);

/**
 * Record a loop event into the thread's log. Fire-and-forget: a run must never
 * die because its record could not be written.
 *
 * @param {object} thread — needs `id`
 * @param {object} entry — { type, ...payload } in the loop's vocabulary
 */
export function recordEvent(thread, entry) {
  const w = activeWriters.get(thread?.id);
  if (!w || !entry?.type) return;
  const { type, turn: _turn, ...payload } = entry;
  if (DROPPED.has(type)) return;
  const mapped = EVENT_MAP[type];
  if (!mapped) return;
  // repair/compaction collapse into `harness` — one type for "the harness acted
  // on its own", with `kind` saying which. A reader scanning the file sees every
  // intervention in one grep instead of needing to know the vocabulary.
  const body = mapped === "harness" ? { kind: type, ...payload } : payload;
  w.appendQuiet(mapped, body);
}
