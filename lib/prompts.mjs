...
AGENTS: Any Claude Code agent can be started as an agnz sub-agent. Pass agent: "<name>" to agent_start and the agent's config is loaded automatically. No need to know where agents are stored — just use the agent name.
...
TYPICAL WORKFLOW:
  1. agent_start(agent: "<name>") — create a thread. Returns thread_id and config.
