// use_skill — load project-local skills on demand. See ADR 0005.
//
...
/**
 * Discover all skills under <cwd>/.claude/skills/. Returns a Map keyed
 * by skill name (from frontmatter, falling back to directory name).
...
export default {
  name: "Skill",
  description:
    "Discover and load project-local skills (instruction sets for specific tasks). Use action=list to see what is available, then action=load to pull in the content of a relevant skill before starting a task.",
  parameters: {
    type: "object",
    properties: {
      action: {
        type: "string",
        enum: ["list", "load"],
        description: "list: show available skills and their descriptions. load: return the full content of a named skill.",
      },
      name: {
        type: "string",
        description: "Skill name to load (required when action=load).",
      },
    },
    required: ["action"],
  ...
  async run(args, ctx) {
    ...
    if (!catalog) {
      catalog = await discoverSkills(cwd);
