...
export default {
  ...
  description:
    ...
    "Writing an existing file requires having Read it in this thread first, and is refused if the file changed on disk " +
    "since that read — Read it again to refresh your view, then retry.",
  parameters: {
    type: "object",
    properties: {
      path: {
  ...
  async run(args, ctx) {
    ...
    // Serialise the clobber-check and the write per file so two concurrent
    // writers cannot both pass the "doesn't exist" check and clobber blind.
    ...
    // Re-validate the parent now that it exists — guards against a symlink
    // being swapped into the tail between resolvePath and now (TOCTOU).
    sandbox.assertInside(dirname(target));
