...
export default {
  name: "Grep",
  description:
    "Search for a pattern across files in the agent's sandbox, returning `path:line: text` lines. Defaults to JavaScript regex; pass literal=true for plain strings. Skips binaries, .git, node_modules, dist, build. Use `include` to restrict to specific file extensions (e.g. '*.mjs', '*.md').",
  parameters: {
    type: "object",
    properties: {
      pattern: {
      ...
      include: {
        type: "string",
        description: "Glob pattern to restrict which files are searched (e.g. '*.mjs', '*.{ts,tsx}'). Matched against the filename only.",
      },
      literal: {
        type: "boolean",
