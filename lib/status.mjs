...
export function deriveInFlight(entries, now = Date.now()) {
  ...
  for (const e of entries || []) {
    if (e.type === "turn_start" || e.type === "thread_start") {
      ...
      // Clearing on ANY llm_call, error included, is what stops a crashed call
      // from reading as one still in flight. Its latency is another matter: a
      // refused connect (milliseconds) or a full timeout would both skew the
      // median that the hung threshold calibrates against, so only calls the
      // model actually answered contribute one.
      pendingStartTs = null;
      if (e.outcome !== "error" && typeof e.latencyMs === "number") latencies.push(e.latencyMs);
    }
  }
