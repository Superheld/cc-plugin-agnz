...
Repo layout follows the standard Claude Code plugin layout: `.claude-plugin/plugin.json` at the root, with `agent/`, `mcp/`, `commands/`, `scripts/`, `hooks/`, `docs/`, `skills/` as siblings. This repo is a **pure plugin repo** — no marketplace manifest. The marketplace lives elsewhere.
