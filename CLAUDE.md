...
| `lib/status.mjs` | The judgment layer of the lead dashboard (ADR 0019). Pure functions over meta + trace: `deriveInFlight` (a trailing `turn_start` without `llm_call` IS an in-flight LLM call; median latency from completed calls), `judgeThread` → the glossary triple `{verdict, evidence, action}` (working/slow/hung/awaiting-*/error/done/stale — hung at max(10 min, 10× the thread's own median)), `fmtDur` (one duration grammar). Consumed by `show`; the CC hooks carry a self-contained mirror (`readRunState`/`isHungRunState` in `scripts/hooks/_lib.mjs` — hooks import nothing from `lib/`). |
...
| `show [<id\|name>]` | The one inspection verb (0.19 zoo cut). No target: list all threads (`--status` filters; recovers dead-runner threads; `list` survives as an undocumented alias). With a target: lean structural view — status, pending, spend, trace stats, no raw transcript. Deeper analysis is log/dashboard territory, not CLI. |
