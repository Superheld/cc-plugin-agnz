...
| `wait <id\|name> [--timeout <s>]` | Poll a detached run until it leaves `running`; prints the outcome (default 300 s timeout; `timeout:true` after — with the phase-labelled `activity` triple `{phase, since, last_action}` as a liveness signal — the runner keeps going). For long runs the skills recommend launching `wait` as a background Bash task so the harness wakes the lead when it exits. |
...
| `show [<id\|name>]` | The one inspection verb (0.19 zoo cut). No target: list all threads (`--status` filters; recovers dead-runner threads; `list` survives as an undocumented alias). With a target: lean structural view — status, pending, spend, trace stats, `filesTouched` (the per-path Write/Edit fold — the diff pointer for review), no raw transcript. Field-tested noise is out: the agentDef collapses to `role`, and `recent` carries agent-side turns only (user-role messages — the lead's own directives — are filtered). Deeper analysis is log/dashboard territory, not CLI. |
| `mailbox [--from x] [--to x] [--kind k] [--limit n]` | Read-only peek into `messages.jsonl` as an interface (never touches the parent cursor). Exists for what the hook does NOT deliver: agent-to-agent traffic in a team, re-reading consumed mail. Text capped like every lean surface. |
