...
| `lib/trace.mjs` | Append-only runtime trace. Writes `<thread-id>.trace.jsonl` alongside the transcript. Event types (ADR 0011 §1): `thread_start` (first run only — tools, model, agent, profile + system prompt), `turn_start` (before every LLM call), `llm_call` (`outcome` ok/error + latency + finishReason + normalized usage; on error also `endpoint` and structured `error.detail` from the client — kind/url/syscall code/HTTP status/body excerpt, so a run that never reached the model is diagnosable), `tool_call` (name, latency, outcome, `target` — the path-ish argument, powering the last-activity liveness display), `repair` (JSON-arg repair), `pause`, `thread_end` (reason + per-run totals). `thread_start` also carries `endpoint` + an `invocation` block (cwd, profile + config layer, agent def, prompt stamp) — never the apiKey. Note that `thread_start` is first-run-only and `thread_end` fires per *run*: there is no run id yet, which is what ADR 0020 is about. Field naming is OpenTelemetry-span-mappable. Always fire-and-forget; failures are silent so tracing never crashes the loop. |
