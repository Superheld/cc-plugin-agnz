...
- **[ADR 0013 — Tool workflow discipline.](./docs/adr/0013-tool-workflow-discipline.md)** The behavioural counterpart to ADR 0012: keep context lean by guiding *how* the agent uses tools. **Grep before Read** (efficiency — locate then read a slice, soft guidance) and **Read before Write/Edit of an existing file** (correctness — never clobber unread content, near-absolute, candidate for hard enforcement). Lives in the harness (`lib/prompts.mjs`). Cache-safe (only smaller appends, never mutates earlier messages) and measurable via evals (does a profile actually Grep-then-slice or dump whole files?). **Proposed** — policy only; the hard Read-before-Edit check is a deferred follow-up.
