...
This file is the in-repo guidance for future Claude sessions working on the codebase. It reflects `main` after the **CLI pivot (0.13.0, ADR 0014)**: the MCP server is gone — the parent drives agnz through `bin/agnz.mjs`. **Implemented:** ADR 0001 (workspace-first), 0002 (mailbox), 0003 (agent defs), 0007 (parent context), 0011 (observability/evals), 0012 (context management — frozen prompt prefix + threshold compaction, see the ADR's 2026-07-23 amendment), 0013 (tool-workflow discipline — Grep→Read→Write, extended by freshness stamps), 0014 (CLI). **Roadmap:** ADR 0004 (board), 0008 (brain). 0005 superseded by 0006; 0009 partially superseded (session-only command model below). ADR 0010 (workspace file manager) was implemented then reverted — its read-before-write kernel now lives in ADR 0013 + the anchor-based Edit tool. ADRs 0001/0004/0010 are referenced but their files were never committed / were reverted. All ADRs live in [`docs/adr/`](./docs/adr/) — see the ADR section at the bottom.
...
| `lib/loop.mjs` | The agent loop. `runThread(ctx)` is the main entry. Handles new messages, resume from pause, drain leftover tool calls. The system prompt is a **frozen prefix** (ADR 0012 phase 1): `renderSystemPrompt` runs once on the first run and the result is snapshotted onto `thread.systemPromptSnapshot`, reused byte-identically every turn so the prefix never grows and the server can cache it. `drainTopOfTurnContext` runs at the top of every turn and injects, as a single synthetic user message, (a) one-time project-context files for newly-visited subdirs (`pendingDirMds` — persisted on thread meta with `visitedDirs`, so a resume can't re-inject; per directory CLAUDE.md wins, else AGENTS.md — `readProjectMd`) and (b) new mailbox mail (ADR 0002) for `agentName`, advancing `inboxCursor`. Combined into one message so two consecutive user turns never occur. The turn top also checks `shouldCompact` (see `lib/compaction.mjs`). ADR 0013 state per thread: `knownFiles` (path list, read-before-write gate) + `fileStamps` ({mtimeMs, size, full} per file — a Write on a file changed since the last read is blocked as stale, Edit exempt by design since its anchor match catches staleness; a full re-read of an unchanged fully-known file is answered "unchanged since your last read" instead of re-sending content, traced `outcome:"deduped"`). |
| `lib/compaction.mjs` | Context compaction (context-diet 3/3, ADR 0012 amendment). Opt-in via the profile's `contextWindow`: when the last observed prompt size crosses `compactThreshold` (default 0.9) of the window, the run pays one deliberate cache reset — `performCompaction` has the model summarize its own session from a ballast-stripped one-shot input (Write/Edit args collapse to path + size, tool results capped; the live transcript is never mutated), appends the summary as a user-role `_compact` marker — with the last few exchanges packed into the marker as inline text (`renderRecentTail`; never as separate messages, which would raise alternation problems) — and clears the knowledge state (`knownFiles`/`fileStamps`/`visitedDirs`/`pendingDirMds`) so the ADR 0013 gates force fresh reads. `buildMessages` sends system prefix + everything from the newest marker. Failure is contained (traced `compaction outcome:"error"`, one attempt per run). |
| `lib/status.mjs` | The judgment layer of the lead dashboard (ADR 0019). Pure functions over meta + trace: `deriveInFlight` (a trailing `api_request` without `api_response` IS an in-flight LLM call; median latency from completed calls), `deriveActivity` (the phase-labelled liveness triple `{phase: generating\|tool\|idle, since, last_action}` — built because the tool-call-only `lastActivity` froze during long generations and "generiert" was indistinguishable from "hängt"), `judgeThread` → the glossary triple `{verdict, evidence, action}` (working/slow/hung/awaiting-*/error/done/turn-limit/stale — hung at max(10 min, 10× the thread's own median); turn-limit so a flail-out never reads like a clean finish), `fmtDur` (one duration grammar). Consumed by `show`/`wait`; the CC hooks carry a self-contained mirror (`readRunState`/`isHungRunState` in `scripts/hooks/_lib.mjs` — hooks import nothing from `lib/`; the block tags in-flight calls `· generating 84s` and prefixes turn-limit summaries with ⚠). |
...
| `lib/plugin-version.mjs` | Reads the plugin's declared version from the manifest (cached per process, never fatal). Paired with `promptFingerprint()` in `prompts.mjs` — an 8-char digest of the prompt templates — to stamp a thread when its system prompt is frozen. The digest is the load-bearing half: agnz bumps the version only at release, so a development build and the released one share a version string while their prompts differ. `show` reports a mismatch as `promptDrift` with a typeable action, making ADR 0012's write-once snapshot visible instead of silent. |
| `lib/run-log.mjs` | **The one append-only record per thread** (ADR 0020) — `<id>.log.jsonl`, replacing the old `<id>.jsonl` transcript + `<id>.trace.jsonl` pair. Envelope `{seq, ts, run, turn, type}`; event types: `run_start` (pid, trigger, endpoint, profile + config layer, agent def, prompt stamp, tools), `message` (the wire truth — this IS the transcript), `api_request` / `api_response` / `api_error` (the last with endpoint + structured client detail: kind/url/syscall code/HTTP status/body), `tool_exec` (name, target, outcome, latency, `refSeq` to the message holding the output), `harness` (`kind`: repair/compaction/…), `pause`, `run_end`. Two rules keep it from re-creating the redundancy it removed: **an `api_request` carries no messages** — it names a seq range plus the prefix digest, and replaying that range against `<id>.system.txt` reproduces the request byte-for-byte (logging bodies in full would be quadratic — the request *is* the conversation); and **observability references, never restates**. Exports `openThreadLog` (single writer per thread, seq seeded from disk), `readThreadLog`, `projectMessages` (what `buildMessages` replays), `reconstructRequest`, `projectApiCalls` (an unanswered request surfaces as `outcome:"unanswered"` instead of vanishing), `recordEvent` (the loop's 27 recording sites translate through here, which is also what guarantees one writer owns the sequence), and `appendWorkspaceLog`/`readWorkspaceLog` for events with no thread to belong to (`workspace.log.jsonl` — `config test`'s server contact was the incident: it printed to stderr and exited, so the one command run *because* the model is not answering recorded nothing). Fire-and-forget for observability, throwing for `message` (losing one would corrupt what the model is replayed from). |
| `lib/trace-stats.mjs` | Zero-dep aggregator over the unified log (ADR 0011 §2, retargeted by ADR 0020). Turns are counted from `api_request` — a turn the model never answered still happened — and `api_error` folds into `llmErrors`, deliberately out of the latency mean (a 2 ms connect refusal would halve it). `aggregateTrace` (pure) folds one thread's events into turns/tokens/latency/tool-outcomes/repair-rate; `aggregateWorkspace` rolls up all traced threads with per-model/per-agent breakdowns. Re-aggregating the file is the authoritative cumulative view across resumes (more accurate than a single run's `thread_end.totals`). Since the 0.19 zoo cut this is a library, not a lead-facing surface: `show` folds `aggregateTrace` in per thread; workspace-wide analysis is external-tooling territory (Bruce's dashboard reads the JSONL files directly). The standalone CLI mode (`node lib/trace-stats.mjs`) still works for terminal use. |
...
| *(retired)* `skills/agnz-setup/scripts/companion.mjs` | Deleted 2026-07-23 — the last MCP-era side-channel script. Profile CRUD, mappings, and info live in the CLI now (`agnz config <sub>` / `agnz info`); the `/agnz:setup` skill is a thin wrapper over those verbs. |
| `scripts/hooks/{user-prompt-submit,session-start,pre-tool-use}.mjs` + `_lib.mjs` | Claude Code hook scripts for ADR 0002 §6a/6b + ADR 0011 §3 + ADR 0015. `user-prompt-submit`/`session-start` inject unread `to:parent` messages plus a spend-aware workspace summary into Claude's context. Per active thread the summary reads **card-first** from `meta.card` (turns/tokens/ctxTokens/mission `task`, stamped by the loop at every pause/finish), showing `ctx ~Xk` — the resume weight a `send` re-sends — rather than cumulative tokens; `readThreadSpend` folding the log remains only as the fallback for legacy card-less threads. A `running` thread gets no vital signs at all any more (ADR 0019 amendment): turns, `ctx ~Xk` and the last-tool-call line were removed — the only per-thread exception that still surfaces is the hung alert, because an exception reaches the lead unbidden rather than being read. `decideInjection` gates re-injection on a real change: new parent mail **or** a thread-set fingerprint delta (a thread started/stopped), so a pure structural change pushes even without new mail, and idle threads >24h collapse to one aggregate line. The parent's **byte-offset** cursor into `messages.jsonl` and the fingerprint advance via atomic tmp+rename only after stdout drain, so state never runs ahead of what reached Claude. `pre-tool-use` is the ADR 0015 fence: it blocks direct `Read` of a thread's log and large-context `Grep` (`-A`/`-B`/`-C` > 10) on them, steering the lead toward `show`/`wait`/`send`. Self-contained — no imports from `lib/`. Fast no-op when the current project has no agnz workspace. Wired into Claude Code via `hooks/hooks.json` — auto-enabled when the plugin is installed; scoped to the plugin's lifetime. |
...
| `wait <id\|name> [--timeout <s>]` | Poll a detached run until it leaves `running`; prints the outcome (default 300 s timeout; `timeout:true` after — the liveness triple was removed with the ADR 0019 amendment, a timeout just says "still running" — the runner keeps going). For long runs the skills recommend launching `wait` as a background Bash task so the harness wakes the lead when it exits. |
...
| `show [<id\|name>]` | The overview verb. No target: list all threads (`--status` filters; recovers dead-runner threads; `list` survives as an undocumented alias) — id, name, role, status, summary, nothing folded from the trace. With a target: `status`, `pending`, `summary`, `error` — exactly what `answer`/`approve`/`stop` need. **Deliberately not analytical** (ADR 0019 amendment 2026-07-25): spend, trace stats, `filesTouched`, `recent` turns and the verdict/liveness triples were all removed because they turned the lead into an analyst. |
| `config <sub>` | Config management (ADR 0017), folded in from the retired setup companion: `list`, `add <name> <baseUrl> <model> [apiKey]`, `set <name> <field> <value>` (optional fields incl. `contextWindow`), `remove`, `use` (writes `_default`), `test` (GET `/v1/models` + `seesConfiguredModel`), `mapping list\|set\|remove`. Writes target the user layer; `--project` targets the project override. The `/agnz:setup` skill wraps these verbs. |
| `info` | Environment overview: version, config file paths, effective profiles/mappings with per-value origins, thread count. |
...
`workspace.json` is pure workspace **state** (ADR 0017): `schemaVersion: 2`, timestamps, and `parent: {cursor, offset, threadFingerprint}` — the parent's delivery position, written by the CC hooks after stdout drain (there is no `cursors/` directory any more). `name`/`cwd` were dropped (derivable from the file's location; a stored cwd was observed lying after a project move). Config lives in `config.json` (either layer). Per thread there are **three** files (ADR 0020), deliberately three different things rather than three views of one: `<id>.log.jsonl` (what happened), `<id>.meta.json` (what is true now), `<id>.system.txt` (the frozen prompt prefix, write-once, referenced by digest from every `api_request`). `<id>.jsonl` and `<id>.trace.jsonl` are gone; threads created before the switch fall back to their old transcript on read, so they keep running. `agnz remove` sweeps all of them by the `<id>.` prefix. ADR 0004 will grow board fields on `workspace.json`. Agent definitions live in CC's standard locations (`~/.claude/agents/` and `<cwd>/.claude/agents/`), not under `agnz/`.
...
Verify by running `agnz show` (and the `/agnz:setup` skill should be available).
...
```bash
agnz config add lmstudio-devstral http://localhost:1234/v1 mistralai/devstral-small-2-2512
```
...
- **[ADR 0012 — Context management.](./docs/adr/0012-context-management.md)** The sub-agent loop has no context management: `buildMessages` rebuilds and re-sends the full prompt every turn, and parts *grow* (visited-dir CLAUDE.md in the system prompt → `lib/loop.mjs:682`; unbounded transcript with verbatim tool results up to 512 KiB/1 MiB). This defeats prefix caching and makes server memory climb monotonically even on mechanical tasks. **Phase 1 implemented** (`renderSystemPrompt` snapshot on first run, reused every turn as the frozen prefix; visited-dir CLAUDE.md injected once into history via `pendingDirMds` + `drainTopOfTurnContext` — both persisted on thread meta since 2026-07-23; covered by `tests/context-prefix.test.mjs`). **Phases 2–3 superseded by the implemented threshold compaction** (2026-07-23 amendment in the ADR): steady state never mutates the transcript (cache stays warm); at `compactThreshold` (default 0.9) × the profile-declared `contextWindow`, one deliberate summary-reset via `lib/compaction.mjs` (`_compact` marker; `buildMessages` sends from the newest marker; knowledge state cleared). Covered by `tests/compaction.test.mjs`. Measurable via the log (`api_response.usage.prompt` per turn; `harness kind:"compaction"` events).

- **[ADR 0013 — Tool workflow discipline.](./docs/adr/0013-tool-workflow-discipline.md)** The behavioural counterpart to ADR 0012: keep context lean by guiding *how* the agent uses tools. **Grep before Read** (efficiency — locate then read a slice, soft guidance) and **Read before Write/Edit of an existing file** (correctness — never clobber unread content, near-absolute, candidate for hard enforcement). **Implemented** as harness logic, not just prompt text: a reactive interceptor (`checkWorkflowDiscipline` in `lib/loop.mjs`, called early in `dispatchToolCall`) backed by a per-thread knowledge state (`knownFiles` on thread meta, updated after every successful Read/Write/Edit). On a violation it blocks the tool and injects a `Workflow:` corrective prompt; the block is traced as `tool_call outcome:"blocked"`. Read→Write is a hard block on existing unread files; Grep→Read is a size-gated redirect (`LARGE_READ_BYTES`). **Extended (2026-07-23, context-diet 2/3) by `fileStamps`** — {mtimeMs, size, full} per known file: a Write on a file changed on disk since the agent's last look is blocked as stale (Edit deliberately exempt — its anchor match against disk bytes catches staleness on its own), and a full re-read of an unchanged fully-known file is answered with a one-line "unchanged since your last read" instead of re-sent content (traced `outcome:"deduped"`; slice reads always pass). Cache-safe (only smaller appends, never mutates earlier messages). Covered by `tests/workflow-discipline.test.mjs`. (Complemented by the anchor-based `Edit` tool, which independently requires having read the bytes to be changed.)
...
- **[ADR 0018 — Teams: an ephemeral container, derived contents.](./docs/adr/0018-team-as-derived-state.md)** v2: the team is an explicit, ephemeral container (`<teamId>.team.json`: goal, members with per-team tasks, wake budget, end tracking) — everything inside is derived (defs/metas/cards), nothing registry-duplicated. Per-member threads under one umbrella; team-scoped address book injected at turn start (team-less agents have no peer recipients at all — solo stays byte-identical); auto-wake within the team under a wake budget, lead notified on exhaustion; `kind: "end"` declarations give termination semantics (done = all ended + no unread team mail); agent = named thread within its team, cross-thread identity fenced to ADR 0008. **Proposed, v2 — discussion draft**; validation via a dev→reviewer→tester dogfood on the dashboard project (hub-and-spoke baseline first, then the real team).
- **[ADR 0019 — The lead dashboard.](./docs/adr/0019-lead-dashboard.md)** One status model, judgments instead of readings: `status` is fact, `verdict` is diagnosis (`working`/`slow`/`hung`/`awaiting-*`/`error`/`stale`), every non-quiet line carries `evidence` + a typeable `action`; thresholds self-calibrate against the thread's own trace median. One glossary across all surfaces (`thread_id`/`name`/`role`/`status`/`verdict`/`since`/`last_action`/`evidence`/`action`), one serialization per consumer (CLI JSON, hook line grammar); one event channel — agnz itself publishes `from:"agnz"` errors into `messages.jsonl`. **Accepted, phase 1 implemented** (`lib/status.mjs`, verdict fields in `show`, hung alert in the hook block, agnz-sender events for dead runner + failed compaction). Open: full `collectStatus` struct + both renderers on it, severity-gated healthy one-liner, preflight/server-contact lines, `agent`→`role` rename.

- **[ADR 0020 — One log per thread.](./docs/adr/0020-diagnostics-consolidation.md)** **Accepted, implemented (breaking).** One append-only `<id>.log.jsonl`; every other view is a projection of it (chat history, API log, tool log, stats). `lib/trace.mjs` deleted, `<id>.jsonl` gone. A run is now first-class (run id on every entry, `run_start`/`run_end`) — previously `thread_end` in fact fired per run and `thread_start` only on a thread's first-ever run, so its endpoint data went stale the moment a profile mapping changed. Verified against a live LAN model, not only fixtures. Historical framing below.

  One record per run instead of four partial ones. Triggered by an `EHOSTUNREACH` field incident where the same error was written to three files in three shapes (`meta.error`, `messages.jsonl`, `thread_end`) while the facts a diagnosis needs were in none of them. **Phase 0 implemented (additive, no field changed meaning):** `llm_call` on failure with endpoint + structured client detail, `thread_start` with endpoint + invocation, consumers adjusted so failures do not skew the latency mean (`trace-stats.llmErrors`) or the hung-threshold median (`status.mjs`). **Open, and the point of the ADR:** agnz has **no first-class run** — no run id, `thread_end` actually fires per run, `thread_start` fires first-run-only (so phase 0's endpoint data goes stale the moment a profile mapping changes). Proposes `run_start`/`run_end` + a run id first (non-breaking), consolidation of the three error records second (breaking — takes the dashboard's trace adapter with it). Carries its own counter-model: add the run id everywhere and keep all three records.
