...
- **[ADR 0018 — Team as derived state, mission as a message.](./docs/adr/0018-team-as-derived-state.md)** The team is the set of live threads — roster (name, def-description, current task) derived at read time and injected at turn start alongside the inbox, only when colleagues exist; the epic/goal lives in the lead's task messages, not in any stored field; coordination stays hub-and-spoke, a2a limited to mailbox questions; **agent = named thread** (name is the identity, def is the role, cross-thread agent identity deferred to ADR 0008/brain); mail to idle colleagues is surfaced to the lead, never auto-wakes the recipient. **Proposed — discussion draft**, validation via a dev→reviewer→test dogfood on the dashboard project before implementation.
